...
public class mergeSortJava {
    ...
    public void intercalar(ArrayList<Show> lista, int esquerda, int meio, int direita, int[] comparacoes,
            int[] movimentacoes) {
        ...
        ArrayList<Show> esquerdaLista = new ArrayList<>(n1);
        ArrayList<Show> direitaLista = new ArrayList<>(n2);

        //inicando o primeiro SubArray

        for(int i = 0; i < n1; i++) {
            esquerdaLista.add(lista.get(esquerda + i));
            movimentacoes[0]++;
        }
        ...
        int i = 0;
        int j = 0;
        int indiceFinal = esquerda;

        while (i < n1 && j < n2) {
            comparacoes[0]++;
            Show showEsq = esquerdaLista.get(i);
            Show showDir = direitaLista.get(j);

            boolean menor;
            if (showEsq.getDuration().compareTo(showDir.getDuration()) < 0) {
                menor = true;
            } else if (showEsq.getDuration().compareTo(showDir.getDuration()) > 0) {
                menor = false;
            } else {
                menor = showEsq.getTitle().compareTo(showDir.getTitle()) <= 0;
                comparacoes[0]++;
            }

            if (menor) {
                lista.set(indiceFinal++, showEsq);
                i++;
            } else {
                lista.set(indiceFinal++, showDir);
                j++;
            }
            movimentacoes[0]++;
        }
        ...
        while (i < n1) {
            lista.set(indiceFinal++, esquerdaLista.get(i++));
            movimentacoes[0]++;
        }

        while (j < n2) {
            lista.set(indiceFinal++, direitaLista.get(j++));
            movimentacoes[0]++;
        }
    }
